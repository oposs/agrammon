...
//          this.debug('setMetaParameter(): this=' + this);
          var meta = {};
          meta = this.getMetaData();
          if (!meta) {
              meta = {};
          }
          // this.debug('setMetaParameter(): name='
          //            + this.getName()
          //            + ', key/val='+key+'/'+val);
          // this.debug('setMetaParameter(): meta[key]='
          //            + meta[key]);
          meta[key] = val;
...
//                     + ', key/val='+key+'/'+val);
      ...
      // setMetaOptions: function(i1, i2, val) {
      //     var meta = this.getMetaData();
      //     meta['options'][i1][i2] = val;
      //     this.setMetaData(meta);
      // },

      // setMetaType: function(val) {
      //     var meta = this.getMetaData();
      //     meta['options']['type'] = val;
      //     this.setMetaData(meta);
      // },
...
//          this.debug('Variable.getMetaOptions(): this='+this);
//          this.debug('Variable.getMetaOptions(): var='+this.getName());
//          this.debug('Variable.getMetaOptions(): metaData='+metaData);
          var optArray = this.getMetaData()['options'];
//          this.debug('Variable.getMetaOptions(): optArray='+optArray);
...
//          this.debug('optionsLang='+optArray);
          var i, len=optArray.length;
          for (i=0; i<len; i++) {
...
//          key = key.replace(/ /g,'_');
//          this.debug('Variable.getOptionLabels(): var='+this.getName());
//          this.debug('key='+key);
          var labels, label;
          ...
          for (i=0; i<len; i++) {
              label = optArray[i];
//              this.debug('Variable.getOptionLabels(): label='+label);
              if (label == key) {
                  found = true;
                  break;
              }
...
//              this.debug('Variable.getOptionLabels(): labels='+labels);
          }
          return labels;
      },
      ...
      clone: function(name) {
          var newVar =
              new agrammon.module.input.Variable().set({
                                                  name:         name,
